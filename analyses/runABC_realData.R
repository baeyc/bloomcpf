...
obsSumStat <- readRDS(here::here("data/summStatSet_ObservedData.rds"))
simuSumStat <- readRDS(here::here("data/summStatSet_abcTable.rds")) %>% dplyr::select(!"iteration")

paramsSimu <- readRDS(here::here("data/params_abcTable.rds")) %>%
...
# Rejection methods
rej5 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.05,method="rejection")
rej25 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.025,method="rejection")

# Local linear approaches
locHl5 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.05,method="loclinear")
locHl25 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.025,method="loclinear")

# Nonlinear local approaches
locHl5_tr <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.05,method="loclinear",transf=names_transf,logit.bounds=bounds_transf)
nlh5 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.05,method="neuralnet",MaxNWts=10000,transf=names_transf,logit.bounds=bounds_transf)
nlh25 <- abc(obsSumStat,paramsSimu,simuSumStat,tol=0.025,method="neuralnet",MaxNWts=10000,transf=names_transf,logit.bounds=bounds_transf)

# Adaptive nonlinear approach
m <- nlh25
...
obsSumStat_is <- obsSumStat %>% dplyr::select(all_of(namesNonZeroVar))
...
dcomp_samples <- data.frame()
for (p in names(paramsSimu)){
  ...
  dq <- d %>% group_by(type) %>% summarise(across(everything(), list(mean=~mean(.x,na.rm = TRUE), median=~median(.x,na.rm = TRUE), q2.5 = ~quantile(.x,0.025,na.rm=TRUE), q97.5 = ~quantile(.x,0.975,na.rm=TRUE))))
  names(dq) <- c("type","mean","median","q2.5","q97.5")
  dq$param <- p
  dcomp_quant <- rbind(dcomp_quant,dq)
  dcomp_samples <- rbind(dcomp_samples,d)
}

saveRDS(dcomp_samples,"results_samples_realData.rds")

# Compute weights for random forests using Epanechnikov kernel
Wepanech <- apply(simuSumStat,1,FUN=function(v){sum((v-obsSumStat)^2)})
quW <- quantile(Wepanech,0.05)
W <- ifelse(Wepanech<=quW,(3/4)*(1-(Wepanech/quW)^2),0)

list_results <- lapply(names(paramsSimu), FUN = function(p){
  ...
  names(obsSumStat) <- colnames(dataABCRF[,-1])
  pred <- predict(object = resABCRF, obs=obsSumStat, training = dataABCRF, quantiles = c(0.025,0.5,0.975), paral = T, verbose=F)
  prednoW <- predict(object = resABCRFnoW, obs=obsSumStat, training = dataABCRF, quantiles = c(0.025,0.5,0.975), paral = T, verbose=F)
  #pred$param <- p
  predall <- as.data.frame(rbind(unlist(pred),unlist(prednoW)))
  predall$weights <- c("w","noW")
  return(predall)#list_results <- rbind(list_results,pred)
})
...
obsSumStat <- obsSumStat %>% dplyr::select(matches(namesNonZeroVar))
...
for (paramName in names(params)){
  ...
  pred_0.025 <- predict.gbm(fit_0.025, newdata=obsSumStat)
  pred_0.5 <- predict.gbm(fit_0.5, newdata=obsSumStat)
  pred_0.975 <- predict.gbm(fit_0.975, newdata=obsSumStat)
  pred_mean_l2 <- predict.gbm(fit_mean_l2, newdata=obsSumStat)
  pred_mean_l1 <- predict.gbm(fit_mean_l1, newdata=obsSumStat)
...
saveRDS(results,"results_realData.rds")
